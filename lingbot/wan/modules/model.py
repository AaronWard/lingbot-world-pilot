...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def _get_or_create_freqs(self, device: torch.device) -> torch.Tensor:
        r"""
        Lazily materialize RoPE freqs on the requested device.

        IMPORTANT:
        - freqs is deterministic derived data, not checkpoint state
        - when WanModel is constructed on the meta device, freqs created in __init__
        would also become meta and unusable
        - this helper rebuilds freqs on demand if they are missing, meta, or on the wrong device
        """
        d = self.dim // self.num_heads

        current = getattr(self, "freqs", None)
        if current is not None:
            if isinstance(current, torch.Tensor):
                if (not getattr(current, "is_meta", False)) and current.device == device:
                    return current
    ...
    def __init__(self,
        ...
        # freqs are deterministic derived data; DO NOT materialize here when model may
        # be built on the meta device. They are created lazily in forward().
    ...
    def forward(
        ...
        # Lazily materialize freqs on the real device.
        freqs = self._get_or_create_freqs(device)
